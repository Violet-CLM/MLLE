...
partial class J2TFile : J2File
{
    ...
    public byte[] AlphaSpread; //0: at least one 0 alpha, no partial alphas. 1: every pixel is 255 alpha. 2: at least one partial alpha
    ...
    public J2TFile(string filename)
    {
        ...
        using (BinaryReader binreader = new BinaryReader(File.Open(filename, FileMode.Open, FileAccess.Read, FileShare.Read), encoding))
        {
            ...
            for (short i = 0; i < MaxTiles; i++) AlphaSpread[i] = data1reader.ReadByte();
    ...
    internal void ReadFromTILEDATA(BinaryReader binreader, byte[] TileTypes)
    {
        ...
        for (ushort tile = 1; tile < TotalNumberOfTiles; tile++)
        {
            ...
            AlphaSpread[tile] = (byte)((rawTransBits == 240 && TileTypes[tile] == 0) ? 1 : 0);
                ...
                for (int xx = 0; xx < 32; ++xx)
                    for (int yy = 0; yy < 32; ++yy)
                    {
                        ...
                        if (image != null)
                        {
                            ...
                            if (color == 1) color = 0;
                            imageArray[tileIndex] = color;
                            if (color == 0)
                                fullyOpaque8Bit = false; //at least one transparent pixel
                            else
    ...
    public SavingResults Save(string filepath)
    {
        ...
        using (BinaryWriter data4writer = new BinaryWriter(UncompressedData[3], encoding))
        {
            ...
            for (int i = 0; i < MaxTiles * 2; ++i)
                data1writer.Write(i < TileCount ? AlphaSpread[i] : (byte)0);
    ...
    internal bool PlusOnly
    {
        get
        {
            ...
            if (InnerX != 0 || InnerY != 0 || InnerAutoX != 0 || InnerAutoY != 0)
                return true;
            if (TextureColorZeroIsTransparent)
                return true;
            if (Texture != 0)
                return true;
            return false;
