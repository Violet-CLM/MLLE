...
namespace MLLE
{
    ...
    {
        const uint CurrentMLLEData5Version = 0x109;
        const string MLLEData5MagicString = "MLLE";
        const string CurrentMLLEData5VersionStringForComparison = "0x109";
        const string CurrentMLLEData5VersionString = "1.9";
        ...
        for (uint i = 0; i < puint; ++i) {{
            ...
            if (pchar >= 0)
                ...
                if (!pbool) {{
                    ...
                    jjCOLORMAP texture(textureWidth, textureHeight);
                    _readImage(data5, texture);
                    texture.makeTexture(layer);
                }}
            }}
